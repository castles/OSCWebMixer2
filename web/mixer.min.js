'use strict';const channelsDiv=document.getElementById("channels"),auxSelect=document.getElementById("aux"),panCheckbox=document.getElementById("panning"),auxiliaries=document.getElementById("auxiliaries");let ws=null,timeout=null,channelInputs=null,panInputs=null;
function sliderChange(a){a=parseFloat(this.value);this.parentNode.style.setProperty("--value",100*a+"%");if(ws){var b="level";this.classList.contains("volumeInput")&&(a=100*a-90);this.classList.contains("panInput")&&(b="pan");sendOSC("/Input_Channels/"+this.dataset.channel+"/Aux_Send/"+auxSelect.options[auxSelect.selectedIndex].dataset.channel+"/send_"+b,[a])}}function sendOSC(a,b=[]){ws.send(JSON.stringify({address:a,args:b}))}
...
function onMessage(a){a=JSON.parse(a.data);if(a.config)buildAux(a.config.aux),buildChannels(a.config.channels),auxSelect.dispatchEvent(new Event("change"));else{var b=a.address.match(/^\/Input_Channels\/([0-9]+)\/Aux_Send\/([0-9]+)\/send_level$/);null!==b&&b[2]==auxSelect.options[auxSelect.selectedIndex].dataset.channel&&(b=channelInputs[b[1]-1],b.value=(parseFloat(a.args[0])+90)/100,b.parentNode.style.setProperty("--value",100*b.value+"%"));b=a.address.match(/^\/Input_Channels\/([0-9]+)\/Aux_Send\/([0-9]+)\/send_pan$/);
null!==b&&b[2]==auxSelect.options[auxSelect.selectedIndex].dataset.channel&&(b=panInputs[b[1]-1],b.value=a.args[0],b.parentNode.style.setProperty("--value",100*b.value+"%"));b=a.address.match(/^\/Input_Channels\/([0-9]+)\/Channel_Input\/name$/);if(null!==b)for(var c of document.querySelectorAll('input[data-channel="'+b[1]+'"]'))c.previousElementSibling.innerHTML=a.args[0];c=a.address.match(/^\/Aux_Outputs\/([0-9]+)\/Buss_Trim\/name$/);if(null!==c){for(let d of auxSelect.options)d.value==c[1]&&(d.innerHTML=
a.args[0],auxSelect.previousElementSibling.innerHTML=auxSelect.getElementsByTagName("option")[auxSelect.selectedIndex].text);for(let d of auxiliaries.getElementsByTagName("button"))if(d.value==c[1]){d.lastChild.nodeValue=a.args[0];break}}}}function formatColour(a){return(a=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(a))?[parseInt(a[1],16),parseInt(a[2],16),parseInt(a[3],16)].join():null}
