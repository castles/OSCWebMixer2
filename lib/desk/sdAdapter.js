...
function createSdAdapter()
{
	return {
		...
		/**
		 * Serialise a neutral DeskEvent back into the SD "DiGiCo iPad" message
		 * shape - the exact inverse of parseIncoming(). The server uses this shape
		 * as its internal representation regardless of which console is connected,
		 * so the cache, the WebSocket protocol and buildConfig() never change.
		 *
		...
		serializeEvent(event)
		{
			...
			{
				case "channelCount":
					return { address: "/Console/Input_Channels", args: [event.count] };
				case "auxModes":
					return { address: "/Console/Aux_Outputs/modes", args: event.modes.slice() };
				case "auxName":
					return { address: `/Aux_Outputs/${event.aux}/Buss_Trim/name`, args: [event.name] };
				case "channelName":
					return { address: `/Input_Channels/${event.channel}/Channel_Input/name`, args: [event.name] };
				case "sendLevel":
					return { address: `/Input_Channels/${event.channel}/Aux_Send/${event.aux}/send_level`, args: [event.db] };
				case "sendPan":
					return { address: `/Input_Channels/${event.channel}/Aux_Send/${event.aux}/send_pan`, args: [event.pan] };
				case "snapshotIndex":
					return { address: "/Snapshots/Current_Snapshot", args: [event.index] };
				case "snapshotName":
					return { address: "/Snapshots/name", args: [event.index, event.name] };
				case "sessionReset":
					return { address: "/Console/Session/!", args: [] };
				default:
					return null;
			}
		}
